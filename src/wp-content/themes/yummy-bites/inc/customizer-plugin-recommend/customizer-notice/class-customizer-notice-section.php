<?php
/**
 * Yummy Bites Customizer Notification Section Class.
 *
 * @package Yummy Bites
 */

/**
 * Yummy_Bites_Customizer_Notice_Section class
 */
class Yummy_Bites_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'yummy_bites_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'yummy_bites_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $yummy_bites_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'yummy_bites_show_recommended_plugins' );

		foreach ( $yummy_bites_recommended_plugins as $slug => $plugin_opt ) {
			...
			$yummy_bites_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$yummy_bites_recommended_plugin['class'] = 'active';
			} else {
				$yummy_bites_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$yummy_bites_recommended_plugin['button_class'] = 'install-now button';
					$yummy_bites_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$yummy_bites_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$yummy_bites_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$yummy_bites_recommended_plugin['button_class'] = 'deactivate-now button';
					$yummy_bites_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$yummy_bites_recommended_plugin['id'] = $slug;
			$yummy_bites_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$yummy_bites_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$yummy_bites_recommended_plugin['description'] = $info->short_description;
			}

			$yummy_bites_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $yummy_bites_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="yummy-bites-recommeded-actions-container yummy-bites-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="yummy-bites-recommeded-actions">
