<?php
/**
 * Yummy Bites Customizer Notification System.
 *
 * @package Yummy Bites
 */
...
class Yummy_Bites_Customizer_Notice {
	...
	/**
	 * The single instance of Yummy_Bites_Customizer_Notice
	 *
	 * @var Yummy_Bites_Customizer_Notice $instance The Yummy_Bites_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Yummy_Bites_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Yummy_Bites_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Yummy_Bites_Customizer_Notice ) ) {
			self::$instance = new Yummy_Bites_Customizer_Notice;
	...
	public function setup_config() {
		...
		$yummy_bites_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$yummy_bites_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'yummy-bites-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'yummy-bites-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), YUMMY_BITES_THEME_VERSION, true );
		wp_localize_script(
			'yummy-bites-customizer-notice', 
            'customizer_notice_data', 
            array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'yummy-bites' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the Yummy_Bites_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Yummy_Bites_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Yummy_Bites_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'yummy_bites_show_recommended_plugins', $show_recommended_plugins );
		}
