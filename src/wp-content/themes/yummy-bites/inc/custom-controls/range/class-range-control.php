...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Yummy_Bites_Range_Slider_Control' ) ) {
	/**
	 ...
	 * @since 1.3.47
	 */
	class Yummy_Bites_Range_Slider_Control extends WP_Customize_Control {
		/**
		 * The control type.
		 *
		 * @access public
		 * @var string
		 */
		public $type = 'yummy-bites-range-slider';
		...
		public function to_json() {
			...
			$this->json['desktop_label'] = __( 'Desktop', 'yummy-bites' );
			$this->json['tablet_label']  = __( 'Tablet', 'yummy-bites' );
			$this->json['mobile_label']  = __( 'Mobile', 'yummy-bites' );
			$this->json['reset_label']   = __( 'Reset', 'yummy-bites' );
		...
		public function enqueue() {
			wp_enqueue_style( 'yummy-bites-range-slider', trailingslashit( get_template_directory_uri() ) . 'inc/custom-controls/range/slider-control.css', null );
			wp_enqueue_script( 'yummy-bites-range-slider', trailingslashit( get_template_directory_uri() ) . 'inc/custom-controls/range/slider-control.js', array( 'jquery', 'customize-base', 'jquery-ui-slider' ), false, true );			
		}
		...
		protected function content_template() {
			?>
			<div class="yummy-bites-range-slider-control  <# if ( ! data.collapsed ) { #>is-active<# } #><# if ( data.group ) { #>_{{ data.group }}<# } #>">
				<div class="yummy-bites-range-title-area">
					<# if ( data.label || data.description ) { #>
						<div class="yummy-bites-range-title-info">
							<# if ( data.label ) { #>
					...
					<div class="yummy-bites-range-slider-controls">
						...
						<span title="{{ data.reset_label }}" class="yummy-bites-reset dashicons dashicons-image-rotate"></span>
					</div>
				</div>

				<div class="yummy-bites-range-slider-areas">
					<# if ( 'undefined' !== typeof ( data.desktop ) ) { #>
						<label class="range-option-area" data-option="desktop" style="display: none;">
							<div class="wrapper <# if ( '' !== data.choices['desktop']['unit'] ) { #>has-unit<# } #>">
								<div class="yummy-bites-slider" data-step="{{ data.choices['desktop']['step'] }}" data-min="{{ data.choices['desktop']['min'] }}" data-max="{{ data.choices['desktop']['max'] }}"></div>

								<div class="yummy_bites_range_value <# if ( '' == data.choices['desktop']['unit'] ) { #>no-unit<# } #>">
					...
					<# if ( 'undefined' !== typeof ( data.tablet ) ) { #>
						<label class="range-option-area" data-option="tablet" style="display:none">
							<div class="wrapper <# if ( '' !== data.choices['tablet']['unit'] ) { #>has-unit<# } #>">
								<div class="yummy-bites-slider" data-step="{{ data.choices['tablet']['step'] }}" data-min="{{ data.choices['tablet']['min'] }}" data-max="{{ data.choices['tablet']['max'] }}"></div>
					...
					<# if ( 'undefined' !== typeof ( data.mobile ) ) { #>
						<label class="range-option-area" data-option="mobile" style="display:none;">
							<div class="wrapper <# if ( '' !== data.choices['mobile']['unit'] ) { #>has-unit<# } #>">
								<div class="yummy-bites-slider" data-step="{{ data.choices['mobile']['step'] }}" data-min="{{ data.choices['mobile']['min'] }}" data-max="{{ data.choices['mobile']['max'] }}"></div>
