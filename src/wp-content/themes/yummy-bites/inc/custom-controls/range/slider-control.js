wp.customize.controlConstructor['yummy-bites-range-slider'] = wp.customize.Control.extend({
	...
	ready: function () {
		...
		var control = this,
			value,
			controlClass = '.customize-control-yummy-bites-range-slider',
			footerActions = jQuery('#customize-footer-actions');

		// Set up the sliders
		jQuery('.yummy-bites-slider').each(function () {
		...
		jQuery(controlClass + ' .yummy_bites_range_value input[type=number]').on('input', function () {
			value = jQuery(this).attr('value');
			if ('' == value) {
				value = -1;
			}
			jQuery(this).closest('label').find('.yummy-bites-slider').slider('value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery(controlClass + ' .yummy-bites-reset').on('click', function () {
			var icon = jQuery(this),
				visible_area = icon.closest('.yummy-bites-range-title-area').next('.yummy-bites-range-slider-areas').children('label:visible'),
				input = visible_area.find('input[type=number]'),
				slider_value = visible_area.find('.yummy-bites-slider'),
				visual_value = visible_area.find('.yummy_bites_range_value'),
		...
		jQuery(controlClass + ' .yummy-bites-range-slider-control').each(function () {
			var _this = jQuery(this);
			_this.find('.yummy-bites-device-controls').children('span:first-child').addClass('selected');
			_this.find('.range-option-area:first-child').show();
		});

		// Do stuff when device icons are clicked
		jQuery(controlClass + ' .yummy-bites-device-controls > span').on('click', function (event) {
			var device = jQuery(this).data('option');

			jQuery(controlClass + ' .yummy-bites-device-controls span').each(function () {
				var _this = jQuery(this);
			...
			jQuery(controlClass + ' .yummy-bites-range-slider-areas label').each(function () {
				var _this = jQuery(this);
				if (device == _this.attr('data-option')) {
					_this.show();
					_this.siblings().hide();
