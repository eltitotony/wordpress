(function ($, api) {
	...
	api.controlConstructor['spacing'] = api.Control.extend({
		ready: function () {
			var control = this;
			$('.yummy-bites-number-control', control.container).on('change keyup',
				function () {
	...
	api.controlConstructor['yummy-bites-spacing-slider'] = api.Control.extend({
		ready: function () {
			var control = this;
	...
	api.controlConstructor['yummy-bites-spacing'] = api.Control.extend({
		ready: function () {
			var control = this,
				controlClass = '.customize-control-yummy-bites-spacing',
				footerActions = jQuery('#customize-footer-actions');

			// Figure out which device icon to make active on load
			jQuery(controlClass + ' .yummy-bites-spacing-control-section').each(function () {
...
jQuery(document).ready(function ($) {
	$('.yummy-bites-link-spacing').on('click', function (e) {
		e.preventDefault();
		...
		_this.parent('.yummy-bites-spacing-section').prevAll().slice(0, 4).find('input').addClass('linked-values').attr('data-element', element);
	...
	$('.yummy-bites-unlink-spacing').on('click', function (e) {
		e.preventDefault();
		...
		_this.parent('.yummy-bites-spacing-section').prevAll().slice(0, 4).find('input').removeClass('linked-values').attr('data-element', '');
	...
	$('.yummy-bites-spacing-section').on('input', '.linked-values', function () {
		var _this = $(this),
